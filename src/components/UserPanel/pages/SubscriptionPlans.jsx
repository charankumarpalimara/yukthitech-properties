...
const PlanCard = ({ plan, billingCycle, onBuy, isTopUp, isActive, isUpgrade, isDowngrade }) => {
  ...
  return (
    ...
    >
      {/* Top Tag */}
      {isPopular && (
        <div className={`absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border shadow-sm ${isPremium
          ? 'bg-gold text-[#011f16] border-gold-300'
        ...
        {(plan.features || []).map((f, i) => {
          ...
          return isInc ? (
            ...
            <div key={i} className="flex items-center gap-2.5 opacity-40">
              ...
              <span className={`text-sm font-semibold line-through ${isPremium ? 'text-slate-400' : 'text-slate-500'}`}>{name}</span>
            </div>
          );
        })}
      </div>
