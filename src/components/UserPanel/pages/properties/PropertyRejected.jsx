...
export default function PropertyRejected() {
  ...
  const handleResubmit = (property) => {
    navigate(`/profile/create-property/${property._id}?resubmit=true`);
  };

  const PropertyCard = ({ prop, onResubmit }) => {
    return (
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden flex flex-col group hover:-translate-y-1 hover:shadow-lg transition-all duration-300">
        ...
        <div className="relative aspect-[4/3] bg-slate-50 overflow-hidden border-b border-slate-100">
          ...
          <div className="absolute top-4 left-4">
            <span className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border shadow-sm bg-rose-50 text-rose-700 border-rose-200/60">
              Rejected
            </span>
          </div>
        </div>
          ...
          <div>
            ...
            {prop.rejectionReason && (
              <div className="bg-rose-50/50 border border-rose-100 rounded-xl p-3 mb-4">
                ...
                <p className="text-xs font-semibold text-rose-600 leading-normal line-clamp-2">{prop.rejectionReason}</p>
              </div>
            )}
          </div>
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-white px-6 py-5 rounded-3xl border border-slate-200/60 shadow-sm gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900 leading-none">Rejected Properties</h2>
          <p className="text-md text-slate-500 mt-2 font-medium">
            Properties that were not approved and may need corrections
          </p>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3 px-4 py-2 bg-rose-50 border border-rose-200 rounded-xl">
            <X size={18} className="text-rose-600 shrink-0" strokeWidth={1.5} />
            <div>
              <p className="text-xs font-bold text-rose-700 leading-none">Total Rejected</p>
      ...
      <div className="bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 px-4">
            ...
            <h3 className="text-lg font-bold text-slate-800">No rejected properties found</h3>
            <p className="text-slate-400 text-sm mt-2 max-w-xs mx-auto">No listings require corrections at this time.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
            {list.map((prop) => (
