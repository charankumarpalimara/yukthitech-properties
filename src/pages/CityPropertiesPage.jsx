...
import { getPropertiesByCity } from '../service/searchService';
import PropertyCard from '../components/PropertyCard/PropertyCard';
import GlobalFilters from '../components/GlobalFilters/GlobalFilters';
import { ChevronL, SearchIco, PinIco, FilterIco, CloseIco, IconCheckCircle } from '../data/icons';
import { CITY_HUB_DETAILS, CITIES } from '../data/constants';
import { getImageUrl, resolvePropertyImage } from '../utils/share';
import { getCompletionPercentage } from '../utils/propertyCompletion';
import { cityPagePath } from '../utils/preloadRoutes';

const formatCityDisplayName = (name) =>
  (name || '')
    .split(' ')
    .filter(Boolean)
...
const CityPropertiesPage = () => {
  ...
  const { popularCities: reduxPopularCities, cities: searchCities } = useSearch();
  const fallbackCities = searchCities && searchCities.length > 0 ? searchCities : CITIES;
  const [popularCities, setPopularCities] = useState([]);

  useEffect(() => {
    const loadPopularCities = async () => {
      if (reduxPopularCities.length > 0) {
        setPopularCities(reduxPopularCities);
        return;
      }
      try {
        const response = await apiClient(`${API_URL}/dynamic-section?productsLimit=10`);
        const data = await response.json();
        if (data.success && data.popularCities?.length) {
        ...
        if (fallbackCities.length) setPopularCities(fallbackCities);
      }
    };
    loadPopularCities();
  }, [reduxPopularCities, fallbackCities]);

  const switcherCities = useMemo(() => {
    const list = [...popularCities];
    const hasCurrent = list.some((c) => c.name?.toLowerCase() === cityName?.toLowerCase());
    if (!hasCurrent && cityName) {
      list.unshift({ name: cityName, image: null, propertyCount: 0 });
    }
    return list;
  }, [popularCities, cityName]);

  const cityData = useMemo(
    () =>
      popularCities.find((c) => c.name?.toLowerCase() === cityName?.toLowerCase()) ||
      fallbackCities.find((c) => c.name?.toLowerCase() === cityName?.toLowerCase()),
    [popularCities, fallbackCities, cityName]
  );
  ...
  useEffect(() => {
    const fetchCityProperties = async () => {
      ...
      try {
        const searchParams = {
          status: 'verified', // Listing verification status
          propertyStatus: filters.status.length > 0 ? filters.status : undefined, // Ready to move / Under construction
        ...
        const result = await getPropertiesByCity(cityName, searchParams);
        ...
        if (result.success) {
          ...
          const normalized = rawData.map((p) => ({
            ...
            propertyTypeId: p.propertyType?._id,
    ...
    fetchCityProperties();
  }, [cityName, debouncedSearchQuery, filters, sortBy]);
  ...
  useEffect(() => {
    ...
    if (document.activeElement !== searchInputRef.current) {
      ...
      if (element) {
        const offset = 120; // height of sticky navbar + top nav
  ...
  // Scroll to top when switching cities
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [cityName]);
  ...
  const cityInfo = {
    ...
    heroImg:
      cityData?.image || cityData?.img
        ? getImageUrl((cityData.image || cityData.img).replace('w=400', 'w=1600'))
  ...
  return (
    <div className="city-page min-h-screen bg-[#f8fafc] font-sans antialiased">
      {/* TOP NAVIGATION */}
      <nav className="w-full bg-white border-b border-slate-200 sticky top-[var(--navbar-height)] z-[100] px-[22px] py-4">
        <div className="max-w-[1350px] mx-auto flex items-center justify-between">
          <Link
            to="/"
            className="inline-flex items-center gap-2.5 text-slate-600 hover:text-gold transition-all text-sm font-semibold no-underline group"
          >
            <div className="w-7 h-7 rounded-full bg-[#f8fafc] border border-[#f1f5f9] flex items-center justify-center group-hover:bg-gold/15 group-hover:border-gold/30 transition-colors">
          ...
          <div className="flex items-center gap-3 relative" ref={citySwitcherRef}>
            <button
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-100 text-xs font-semibold text-slate-500 hover:bg-gold/10 hover:text-gold hover:border-gold/30 transition-all cursor-pointer"
              onClick={() => setIsCitySwitcherOpen(!isCitySwitcherOpen)}
            >
              <PinIco className="w-3 h-3" />
              Change City: <span className="text-slate-900">{formatCityDisplayName(cityName)}</span>
              <ChevronL
                className={`w-2.5 h-2.5 transition-transform duration-300 ${isCitySwitcherOpen ? '-rotate-90' : 'rotate-90'}`}
              />
            </button>
            ...
            {isCitySwitcherOpen && (
              <div className="absolute top-[calc(100%+8px)] right-0 w-[240px] bg-white rounded-[20px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-[#f1f5f9] p-2 z-[110] animate-in fade-in slide-in-from-top-2">
                ...
                <div className="max-h-[300px] overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                  {switcherCities.length > 0 ? (
                    switcherCities.map((city) => {
                      const label = formatCityDisplayName(city.name);
                      const isActive = city.name?.toLowerCase() === cityName?.toLowerCase();
                      return (
                        <button
                          key={city.name}
                          type="button"
                          className={`w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${isActive ? 'bg-gold/10 text-gold' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}
                          onClick={() => {
                            setIsCitySwitcherOpen(false);
                            navigate(cityPagePath(city.name));
                          }}
                        >
                          <span className="truncate text-left">{label}</span>
                          <span className="flex items-center gap-1.5 shrink-0 text-xs font-medium text-slate-400">
                            {city.propertyCount > 0 && <span>{city.propertyCount}</span>}
                            {isActive && <IconCheckCircle size={14} className="text-gold" />}
                          </span>
                        </button>
                      );
                    })
                  ) : (
                    <p className="px-3 py-4 text-sm text-slate-500">No cities available.</p>
                  )}
                </div>
              </div>
            )}
        ...
        <div className="max-w-[1350px] mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-6 relative z-10">
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold text-white m-0 tracking-tight">
              Properties in <span className="text-gold">{formatCityDisplayName(cityName)}</span>
            </h1>
            <p className="mt-2 text-sm text-slate-300 max-w-xl leading-relaxed">
              {cityInfo.description || `Explore handpicked, premium verified residential and commercial opportunities in ${formatCityDisplayName(cityName)}.`}
            </p>
          </div>

          <div className="flex flex-col gap-2 w-full max-w-sm shrink-0">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Search Project</label>
          ...
          <div className="flex-1 min-w-0 w-full">
                ...
                <div className="flex items-center h-[42px] bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  ...
                  >
                    ...
                    {(filters.beds.length > 0 ||
                      ...
                      filters.maxPrice < 500000000 ||
                      filters.minArea > 0 ||
                      filters.vastu) && <span className="w-1.5 h-1.5 bg-gold rounded-full" />}
                  </button>
            ...
            <div className="relative min-h-[280px] w-full">
              ...
              {isInitialLoading && properties.length === 0 ? (
                ...
                >
                  {properties.map((p) => (
                    <PropertyCard key={p.id} property={p} variant="variation3" />
                  ))}
                </div>
              ) : (
                <div
                ...
                >
                  ...
                  <p className="text-sm text-slate-500 mb-6">
                    ...
                    {formatCityDisplayName(cityName)}.
                  </p>
                  <button
                    type="button"
